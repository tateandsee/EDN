...
async function main() {
  try {
    console.log('Starting creation of 30 NSFW models data...')
    ...
    console.log('Creating 15 semi-nude models...')
    for (let i = 0; i < 15; i++) {
      console.log(`Creating semi-nude model ${i + 1}/15...`)
      const modelData = generateModelData(false, i)
      
      try {
        await createMarketplaceItem(modelData, aiLegendUser.id, placeholderPath)
        generatedModels.push(modelData)
      } catch (error) {
        console.error(`Failed to create semi-nude model ${i + 1}:`, error)
        // Continue with next model
      }
    }

    // Generate 15 full-nude models
    console.log('Creating 15 full-nude models...')
    for (let i = 0; i < 15; i++) {
      console.log(`Creating full-nude model ${i + 1}/15...`)
      ...
      try {
        ...
        console.error(`Failed to create full-nude model ${i + 1}:`, error)
        // Continue with next model
      }
    }

    console.log(`\nCreation complete! Created ${generatedModels.length} models:`)
