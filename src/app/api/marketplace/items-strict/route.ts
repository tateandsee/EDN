...
export async function GET(request: NextRequest) {
  try {
    ...
    const validateImages = searchParams.get('validateImages') === 'true'
    ...
    for (const item of items) {
      try {
        ...
        let imageValidation = { isValid: true, errors: [] as string[] }
        if (validateImages) {
          ...
          // Skip items with invalid images if strict validation is enabled
    ...
    return NextResponse.json({
      ...
      metadata: {
        validateImages,
        imageValidationEnabled: validateImages,
        totalProcessed: processedItems.length,
        totalSkipped: items.length - processedItems.length
      }
    })
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Process image files with strict validation
    const imageUrls: string[] = []
    let validatedThumbnail = thumbnail
    ...
    if (thumbnail) {
      const thumbnailValidation = await imageIntegrityService.validateImage(thumbnail)
      if (!thumbnailValidation.isValid) {
        return NextResponse.json(
          { error: `Thumbnail validation failed: ${thumbnailValidation.error}` },
          { status: 400 }
        )
      }
      
      // Use corrected URL if available
      if (thumbnailValidation.correctedUrl) {
        validatedThumbnail = thumbnailValidation.correctedUrl
      }
    }
    ...
    if (imageUrls.length === 0 && !validatedThumbnail) {
      return NextResponse.json(
    ...
    const item = await db.marketplaceItem.create({
      data: {
        ...
        currency: 'USD',
        thumbnail: validatedThumbnail || undefined,
    ...
    return NextResponse.json({ 
      ...
      metadata: {
        ...
        thumbnailValidated: !!validatedThumbnail
      }
    }, { status: 201 })
  } catch (error) {
